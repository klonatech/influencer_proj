...
class GuestOnlyView(View):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if request.session.get('SignUp_done'):
            print('DONE')
            messages.success(request, 'You have signed up. To activate the account, contact the admin.')
...
class LogInView(GuestOnlyView, FormView):
    ...
    def dispatch(self, request, *args, **kwargs):
        # if request.session.get('SignUp_done'):
        #     request.session['SignUp_done'] = ''
        # Sets a test cookie to make sure the user has cookies enabled
        request.session.set_test_cookie()
        if 'SignUp_done' in request.session.keys():
...
class SignUpView(GuestOnlyView, FormView):
    ...
    def form_valid(self, form):
        ...
        # user_table.create_collection_master_table_for_user()
        # user_table.create_collection_result_detail_for_user()
        user.save()
        ...
        if settings.ENABLE_USER_ACTIVATION:
            ...
            messages.success(
                request, 'You are signed up. To activate the account, contact the admin.')
            request.session['SignUp_done'] = 'yes'
...
class ResendActivationCodeView(GuestOnlyView, FormView):
    template_name = 'accounts/activation-code.html'
